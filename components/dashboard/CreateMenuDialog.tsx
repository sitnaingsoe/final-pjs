        ...
        <>
            ...
            {isOpen && mounted && createPortal(
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
                    ...
                    <div className="bg-white/90 backdrop-blur-2xl rounded-[2rem] w-full max-w-lg max-h-[85vh] overflow-y-auto custom-scrollbar flex flex-col relative z-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] border border-white animate-in zoom-in-95 duration-300">
                        <div className="sticky top-0 z-20 bg-gradient-to-r from-white/60 to-white/40 border-b border-gray-100 p-8 flex justify-between items-start backdrop-blur-xl">
                            <div className="flex gap-4 items-start">
                                ...
                                <div>
                                    <h3 className="text-xl font-black text-black tracking-tight">Add Main Product</h3>
                                    <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mt-1">ဟင်းပွဲအသစ်သတ်မှတ်၍ ဆိုင်ခွဲများသို့ တန်းဖြန့်ခြင်း</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-8 bg-white/40">
                            ...
                            <form action={handleSubmit} className="space-y-6">
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest">Menu Image (ပုံ)</label>
                                    <input type="file" name="image" accept="image/*" disabled={isPending} className="w-full bg-white border border-gray-200 rounded-xl p-2 text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-black file:text-white hover:file:bg-gray-900 transition-colors cursor-pointer" />
                                </div>

                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest">Description (အညွှန်း)</label>
                                    <textarea name="description" disabled={isPending} className="w-full h-20 bg-white border border-gray-200 rounded-xl p-4 text-xs focus:outline-none focus:border-black text-black placeholder-slate-400 resize-none transition-colors" placeholder="ဟင်းပွဲအကြောင်း အနည်းငယ်ဖော်ပြပါ..."></textarea>
                                </div>

                                {/* 🎯 ဆိုင်ခွဲများ ရွေးချယ်ရန် Checkbox စာရင်း */}
                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest">Select Branches to Deploy (ဖြန့်ဝေမည့် ဆိုင်ခွဲများ)</label>
                                    <div className="bg-white border border-gray-200 rounded-xl p-3 max-h-32 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 gap-2 custom-scrollbar">
                                        {branches.map(branch => (
                                            <label key={branch.id} className="flex items-center gap-3 text-xs text-gray-700 cursor-pointer select-none hover:text-black transition-colors bg-gray-50/50 p-3 rounded-xl border border-gray-100 hover:border-gray-200">
                                                <input
                                                    type="checkbox"
                                                    checked={selectedBranches.includes(branch.id)}
                                                    onChange={() => handleCheckboxChange(branch.id)}
                                                    className="accent-black w-4 h-4 rounded border-gray-300 bg-white cursor-pointer"
                                                    disabled={isPending}
                                                />
                                        ...
                                        {branches.length === 0 && (
                                            <div className="col-span-1 sm:grid-cols-2 text-xs text-gray-400 font-bold p-2 text-center py-4 bg-gray-50 rounded-xl border border-dashed border-gray-200 uppercase tracking-widest">ဆိုင်ခွဲများ မရှိသေးပါ</div>
                                        )}
                                    </div>
                                </div>

                                <div className="flex justify-end gap-3 pt-6 border-t border-gray-200/50">
                                    <button type="button" onClick={() => setIsOpen(false)} className="bg-white hover:bg-gray-50 border border-gray-200 text-gray-600 hover:text-black font-bold px-6 py-3 rounded-xl text-[10px] uppercase tracking-widest transition-colors shadow-sm">Cancel</button>
