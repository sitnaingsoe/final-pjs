                ...
                <div className="bg-slate-900/50 p-5 rounded-2xl border border-slate-800 min-h-[500px] flex flex-col">
                    ...
                    <div className="space-y-4 flex-1 overflow-y-auto pr-1 custom-scrollbar">
                        {pendingOrders.map(order => (
                            <div key={order.id} className="bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-4 hover:border-red-500/50 transition-colors shadow-lg shadow-black/20 group animate-in fade-in slide-in-from-bottom-2">
                                ...
                                <button onClick={() => handleStatusChange(order.id, OrderStatus.COOKING)} className="w-full bg-slate-800 hover:bg-red-500 hover:text-white text-slate-300 text-xs font-black py-3 rounded-lg transition-all border border-slate-700 hover:border-red-500 flex justify-center items-center gap-2">
                ...
                <div className="bg-slate-900/50 p-5 rounded-2xl border border-slate-800 min-h-[500px] flex flex-col">
                    ...
                    <div className="space-y-4 flex-1 overflow-y-auto pr-1 custom-scrollbar">
                        {cookingOrders.map(order => (
                            <div key={order.id} className="bg-slate-950 p-5 rounded-xl border border-slate-800 space-y-4 hover:border-blue-500/50 transition-colors shadow-lg shadow-black/20 group animate-in fade-in slide-in-from-bottom-2">
                                <div className="flex justify-between items-center border-b border-slate-800/50 pb-3">
                                    <span className="font-black text-sm text-slate-200">#Ord-{order.orderNumber}</span>
                                    <span className="text-xs font-bold text-slate-500 bg-slate-900 px-2 py-1 rounded-md">{new Date(order.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                </div>
                                ...
                                <button onClick={() => handleStatusChange(order.id, OrderStatus.READY)} className="w-full bg-slate-800 hover:bg-blue-500 hover:text-white text-slate-300 text-xs font-black py-3 rounded-lg transition-all border border-slate-700 hover:border-blue-500 flex justify-center items-center gap-2">
                ...
                <div className="bg-slate-900/50 p-5 rounded-2xl border border-slate-800 min-h-[500px] flex flex-col">
                    ...
                    <div className="space-y-4 flex-1 overflow-y-auto pr-1 custom-scrollbar">
                        {readyOrders.map(order => (
                            <div key={order.id} className="bg-slate-950 p-5 rounded-xl border border-green-500/30 space-y-4 shadow-lg shadow-green-500/5 relative overflow-hidden animate-in fade-in slide-in-from-bottom-2">
