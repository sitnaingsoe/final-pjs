                    ...
                    <div className="bg-white rounded-[2rem] w-full max-w-sm relative z-10 shadow-[0_25px_70px_-15px_rgba(0,0,0,0.4)] border border-gray-100 overflow-hidden animate-in zoom-in-95 duration-300">
                        <div className="bg-white border-b border-gray-100 p-6 flex justify-between items-start">
                            <div className="flex gap-4 items-start">
                                ...
                                <div>
                                    <h3 className="text-lg font-black text-black tracking-tight">Reset Password</h3>
                                    <p className="text-[10px] text-gray-400 mt-0.5 font-bold uppercase tracking-widest">{staff.name} အတွက် လျှို့ဝှက်နံပါတ်အသစ်</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-6 bg-white">
                            ...
                            <form action={handlePasswordReset} className="space-y-5">
                                ...
                                <div className="flex justify-end gap-3 pt-6 border-t border-gray-200/50">
                                    <button type="button" onClick={() => setIsResetOpen(false)} className="bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-black px-5 py-2.5 rounded-xl text-xs uppercase tracking-wider font-bold transition-colors shadow-sm">Cancel</button>
            ...
            {isToggleOpen && mounted && createPortal(
                <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
                    ...
                    <div className="bg-white rounded-[2rem] w-full max-w-sm relative z-10 shadow-[0_25px_70px_-15px_rgba(0,0,0,0.4)] border border-gray-100 overflow-hidden animate-in zoom-in-95 duration-300 p-8 text-center">
                        ...
                        <h3 className="text-xl font-black text-black mb-2 tracking-tight">{staff.isActive !== false ? "Suspend Staff?" : "Unsuspend Staff?"}</h3>
                        <p className="text-xs text-gray-500 mb-8 font-bold">
                            {staff.isActive !== false
                                ? <>Are you sure you want to suspend <strong className="text-black uppercase">{staff.name}</strong>? They will not be able to log in.</>
                                : <>Are you sure you want to unsuspend <strong className="text-black uppercase">{staff.name}</strong>? They will regain access.</>
                            }
                        </p>
