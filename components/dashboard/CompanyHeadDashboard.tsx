        ...
        <div className="space-y-6 lg:space-y-8 text-black">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white/60 backdrop-blur-xl border border-gray-100 p-6 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group">
                    ...
                    <p className="text-xs font-black text-gray-500 uppercase tracking-widest relative z-10">Total Revenue</p>
                    <h3 className="text-3xl font-black text-black mt-2 tracking-tight relative z-10">{totalRevenue.toLocaleString()} <span className="text-sm text-gray-400 font-bold uppercase tracking-widest">MMK</span></h3>
                </div>
                <div className="bg-white/60 backdrop-blur-xl border border-gray-100 p-6 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-opacity">
                    ...
                    <p className="text-xs font-black text-gray-500 uppercase tracking-widest relative z-10">Avg Order Value</p>
                    <h3 className="text-3xl font-black text-black mt-2 tracking-tight relative z-10">{averageOrderValue.toLocaleString()} <span className="text-sm text-gray-400 font-bold uppercase tracking-widest">MMK</span></h3>
                </div>
                <div className="bg-white/60 backdrop-blur-xl border border-gray-100 p-6 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-opacity">
                    ...
                    <p className="text-xs font-black text-gray-500 uppercase tracking-widest relative z-10">Active Branches</p>
                    <h3 className="text-3xl font-black text-black mt-2 tracking-tight relative z-10">{totalBranches}</h3>
                </div>
                <div className="bg-white/60 backdrop-blur-xl border border-gray-100 p-6 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-opacity">
                    ...
                    <p className="text-xs font-black text-gray-500 uppercase tracking-widest relative z-10">Total Orders</p>
                    <h3 className="text-3xl font-black text-black mt-2 tracking-tight relative z-10">{totalOrders}</h3>
                </div>
            </div>

            {/* 📊 ၁။ ဂရပ်များကို အလယ်တွင် ပြသခြင်း */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
                <div className="bg-white/60 backdrop-blur-xl border border-gray-100 p-6 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
            ...
            <div className="bg-white/60 backdrop-blur-xl border border-gray-100 rounded-[2rem] p-6 lg:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 md:mb-8">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-base font-black uppercase tracking-wider text-black">Branch Performance</h2>
                    </div>
                    <button onClick={handleDownloadCSV} className="group relative bg-black text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-[0_4px_12px_rgb(0,0,0,0.1)] hover:shadow-[0_4px_16px_rgb(0,0,0,0.2)] hover:-translate-y-0.5 flex items-center gap-2 overflow-hidden">
                            ...
                            {branchTableData.map((branch) => (
                                <tr key={branch.id} className="hover:bg-black/5 transition-colors group">
                                    <td className="py-4 pl-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center text-black font-black text-xs shrink-0 group-hover:bg-black group-hover:text-white transition-colors">
                                                {branch.name.substring(0, 1)}
                                            </div>
                                            <span className="font-bold text-gray-900">{branch.name}</span>
                                        </div>
                                    </td>
                                    <td className="py-4 text-gray-500 font-medium">{branch.manager}</td>
                                    <td className="py-4 text-center font-mono font-medium text-gray-900">{branch.totalOrders}</td>
                                    <td className="py-4 text-right font-mono font-medium text-gray-600">{branch.aov.toLocaleString()}</td>
                                    <td className="py-4 text-right font-mono font-black text-black">{branch.revenue.toLocaleString()}</td>
                                    <td className="py-4 text-right pr-4">
                                        <div className="flex items-center justify-end gap-3">
                                            <span className="font-mono text-gray-900 font-bold">{branch.contributionMargin}%</span>
