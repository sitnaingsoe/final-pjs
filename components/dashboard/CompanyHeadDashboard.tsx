...
export default function CompanyHeadDashboard({ data }: CompanyDashboardProps) {
    const { 
        companyName = 'BiteCraft Food Group', 
    ...
    const handleDownloadCSV = () => {
        ...
        link.download = `hq_branch_performance_${new Date().toISOString().split('T')[0]}.csv`
        link.click()
    }

    // Top Performers calculation
    const sortedByRevenue = [...branchTableData].sort((a, b) => b.revenue - a.revenue)
    const topRevenueBranch = sortedByRevenue[0]

    const sortedByOrders = [...branchTableData].sort((a, b) => b.totalOrders - a.totalOrders)
    const topOrdersBranch = sortedByOrders[0]

    const sortedByAOV = [...branchTableData].sort((a, b) => b.aov - a.aov)
    const topAovBranch = sortedByAOV[0]
        ...
        <div className="space-y-6 lg:space-y-8 text-foreground">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="glass border border-border/50 p-6 rounded-[2rem] shadow-lg relative overflow-hidden group">
                    ...
                    <p className="text-xs font-black text-muted-foreground uppercase tracking-widest relative z-10">Total Net Revenue</p>
                ...
                <div className="glass border border-border/50 p-6 rounded-[2rem] shadow-lg relative overflow-hidden group">
                    ...
                    <p className="text-xs font-black text-muted-foreground uppercase tracking-widest relative z-10">Avg Ticket Size (AOV)</p>
                ...
                <div className="glass border border-border/50 p-6 rounded-[2rem] shadow-lg relative overflow-hidden group">
                    ...
                    <p className="text-xs font-black text-muted-foreground uppercase tracking-widest relative z-10">Operating Branches</p>
                ...
                <div className="glass border border-border/50 p-6 rounded-[2rem] shadow-lg relative overflow-hidden group">
                    ...
                    <p className="text-xs font-black text-muted-foreground uppercase tracking-widest relative z-10">Settled Invoices</p>
                    <h3 className="text-3xl font-black text-foreground mt-2 tracking-tight relative z-10">{totalOrders}</h3>
                </div>
            </div>

            {/* 🏆 Leaderboard Highlights */}
            {branchTableData.length > 0 && totalRevenue > 0 && (
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                    {topRevenueBranch && (
                        <div className="p-4 rounded-2xl bg-gradient-to-tr from-amber-500/15 via-orange-500/10 to-transparent border border-orange-500/30 flex items-center gap-3.5">
                            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-orange-500 to-amber-500 text-white flex items-center justify-center font-bold shrink-0 shadow-md shadow-orange-500/20">
                                🏆
                            </div>
                            <div className="overflow-hidden">
                                <span className="text-[10px] font-black uppercase tracking-widest text-orange-600 dark:text-orange-400 block">Top Revenue Branch</span>
                                <p className="text-sm font-black text-foreground truncate">{topRevenueBranch.name}</p>
                                <p className="text-[11px] font-mono text-muted-foreground font-bold">{topRevenueBranch.revenue.toLocaleString()} MMK ({topRevenueBranch.contributionMargin}%)</p>
                            </div>
                        </div>
                    )}

                    {topOrdersBranch && (
                        <div className="p-4 rounded-2xl bg-gradient-to-tr from-blue-500/15 via-sky-500/10 to-transparent border border-blue-500/30 flex items-center gap-3.5">
                            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-blue-500 to-sky-500 text-white flex items-center justify-center font-bold shrink-0 shadow-md shadow-blue-500/20">
                                ⚡
                            </div>
                            <div className="overflow-hidden">
                                <span className="text-[10px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 block">Highest Order Volume</span>
                                <p className="text-sm font-black text-foreground truncate">{topOrdersBranch.name}</p>
                                <p className="text-[11px] font-mono text-muted-foreground font-bold">{topOrdersBranch.totalOrders} paid orders</p>
                            </div>
                        </div>
                    )}

                    {topAovBranch && (
                        <div className="p-4 rounded-2xl bg-gradient-to-tr from-purple-500/15 via-indigo-500/10 to-transparent border border-purple-500/30 flex items-center gap-3.5">
                            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-purple-500 to-indigo-500 text-white flex items-center justify-center font-bold shrink-0 shadow-md shadow-purple-500/20">
                                💎
                            </div>
                            <div className="overflow-hidden">
                                <span className="text-[10px] font-black uppercase tracking-widest text-purple-600 dark:text-purple-400 block">Highest Average Ticket</span>
                                <p className="text-sm font-black text-foreground truncate">{topAovBranch.name}</p>
                                <p className="text-[11px] font-mono text-muted-foreground font-bold">{topAovBranch.aov.toLocaleString()} MMK per order</p>
                            </div>
                        </div>
                    )}
                </div>
            ...
            {/* 🏪 ၃။ Performance Table & Report Actions */}
            <div className="glass border border-border/50 rounded-[2rem] p-6 lg:p-8 shadow-lg">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 md:mb-8">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-base font-black uppercase tracking-wider text-foreground">Branch Performance & Revenue Share</h2>
                    ...
                    <div className="flex items-center gap-2.5">
                        <button 
                            ...
                            className="px-4 py-2.5 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 text-white rounded-xl text-xs font-bold uppercase tracking-wider transition-all shadow-md shadow-orange-500/20 hover:shadow-lg flex items-center gap-1.5"
                        >
                        ...
                        <button 
                            ...
                            className="px-4 py-2.5 bg-card hover:bg-muted border border-border text-foreground hover:border-orange-500/40 rounded-xl text-xs font-bold uppercase tracking-wider transition-all shadow-sm flex items-center gap-1.5"
                        >
                            ...
                            {branchTableData.map((branch) => (
                                <tr key={branch.id} className="hover:bg-muted/50 transition-colors group">
                                    <td className="py-4 pl-4">
                                        <div className="flex items-center gap-3">
                                            <div className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center text-foreground font-black text-xs shrink-0 group-hover:bg-orange-500 group-hover:text-white transition-colors">
                                    ...
                                    <td className="py-4 text-right pr-4">
                                        <div className="flex items-center justify-end gap-3">
                                            ...
                                            <div className="w-20 h-2 bg-muted rounded-full overflow-hidden shrink-0">
                                                <div 
                                                    className="h-full bg-gradient-to-r from-orange-500 to-amber-500 rounded-full transition-all duration-1000 ease-out relative overflow-hidden" 
