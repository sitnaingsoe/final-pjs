    ...
    return (
        <div className="flex items-center justify-end gap-2">
            ...
            <button
                ...
                className="group relative flex items-center justify-center w-8 h-8 bg-gray-50 hover:bg-black border border-gray-200 hover:border-black text-gray-500 hover:text-white rounded-lg transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5 overflow-hidden"
                title="Edit Menu"
            >
            ...
            <button
                ...
                className={`group relative flex items-center justify-center w-8 h-8 rounded-lg transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5 overflow-hidden border disabled:opacity-50 ${menu.isActive
                    ? 'bg-gray-50 hover:bg-amber-500 border-gray-200 hover:border-amber-500 text-amber-500 hover:text-white'
            ...
            {isEditOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
                    ...
                    <div className="bg-white/90 backdrop-blur-2xl rounded-[2rem] w-full max-w-md relative z-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] border border-white overflow-hidden animate-in zoom-in-95 duration-300">
                        <div className="bg-gradient-to-r from-white/60 to-white/40 border-b border-gray-100 p-6 flex justify-between items-start">
                            <div className="flex gap-4 items-start">
                                ...
                                <div>
                                    <h3 className="text-lg font-black text-black tracking-tight">Edit Main Menu</h3>
                                    <p className="text-[10px] text-gray-400 mt-0.5 font-bold uppercase tracking-widest">ဟင်းပွဲအမည်နှင့် စံနှုန်းစျေးနှုန်း ပြင်ဆင်ခြင်း</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-6 bg-white/40">
                            ...
                            <form action={handleUpdateMenu} className="space-y-5">
                                ...
                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest">Menu Image (ပုံ အသစ်ပြောင်းရန်)</label>
                                    <input type="file" name="image" accept="image/*" disabled={isPending} className="w-full bg-white border border-gray-200 rounded-xl p-2 text-xs text-gray-500 file:mr-4 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-bold file:bg-black file:text-white hover:file:bg-gray-900 transition-colors cursor-pointer" />
                                    {menu.image && (
                                        <p className="text-[10px] text-gray-400 mt-2 font-bold uppercase tracking-widest">ယခုလက်ရှိပုံ: <a href={menu.image} target="_blank" className="text-black underline font-black hover:text-gray-700 ml-1">ကြည့်ရန်</a></p>
                                    )}
                                </div>

                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest">Description</label>
                                    <textarea name="description" defaultValue={menu.description || ''} disabled={isPending} className="w-full h-20 bg-white border border-gray-200 rounded-xl p-3 text-xs focus:outline-none focus:border-black text-black placeholder-slate-400 resize-none transition-colors"></textarea>
                                </div>

                                {/* 🎯 ဆိုင်ခွဲများ ရွေးချယ်ရန် Checkbox စာရင်း */}
                                <div className="space-y-1.5">
                                    <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest">Active Branches (ဖြန့်ဝေမည့် ဆိုင်ခွဲများ)</label>
                                    <div className="bg-white border border-gray-200 rounded-xl p-3 max-h-32 overflow-y-auto space-y-2 custom-scrollbar">
                                        {branches.map(branch => (
                                            <label key={branch.id} className="flex items-center gap-3 text-xs text-gray-700 cursor-pointer select-none hover:text-black transition-colors bg-gray-50/50 p-2.5 rounded-lg border border-gray-100 hover:border-gray-200">
                                                <input
                                                    type="checkbox"
                                                    checked={selectedBranches.includes(branch.id)}
                                                    onChange={() => handleCheckboxChange(branch.id)}
                                                    className="accent-black rounded border-gray-300 bg-white w-4 h-4 cursor-pointer"
                                                    disabled={isPending}
                                                />
                                        ...
                                        {branches.length === 0 && (
                                            <div className="text-xs text-gray-400 font-bold p-2 text-center py-4 bg-gray-50 rounded-lg border border-dashed border-gray-200 uppercase tracking-widest">ဆိုင်ခွဲများ မရှိသေးပါ</div>
                                        )}
                                    </div>
                                </div>

                                <div className="flex justify-end gap-3 pt-6 border-t border-gray-200/50">
                                    <button type="button" onClick={() => setIsEditOpen(false)} disabled={isPending} className="bg-white border border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-black px-5 py-2.5 rounded-xl text-xs uppercase tracking-wider font-bold transition-colors shadow-sm">Cancel</button>
                                    <button type="submit" disabled={isPending} className="bg-black hover:bg-gray-900 text-white px-6 py-2.5 rounded-xl text-xs uppercase tracking-wider font-bold shadow-lg shadow-black/10 hover:shadow-xl transition-all disabled:opacity-50 flex items-center gap-2 hover:-translate-y-0.5">
                                        {isPending ? (
                                            <>
            ...
            {isToggleOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
                    ...
                    <div className="bg-white/90 backdrop-blur-2xl rounded-[2rem] w-full max-w-sm relative z-10 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.3)] border border-white overflow-hidden animate-in zoom-in-95 duration-300 p-8 text-center">
                        ...
                        <h3 className="text-xl font-black text-black mb-2 tracking-tight">{menu.isActive ? "Archive Menu?" : "Activate Menu?"}</h3>
                        <p className="text-xs text-gray-500 mb-8 font-bold">
                            {menu.isActive 
                                ? <>Are you sure you want to archive <strong className="text-black uppercase">{menu.name}</strong>? Sales history will not be deleted.</>
                                : <>Are you sure you want to activate <strong className="text-black uppercase">{menu.name}</strong> to be available in the system?</>
                            }
                        </p>
                        
                        <div className="flex gap-3">
                            <button type="button" onClick={() => setIsToggleOpen(false)} disabled={isPending} className="flex-1 bg-white border border-gray-200 hover:bg-gray-50 text-gray-800 font-bold py-3 rounded-xl transition-colors text-xs uppercase tracking-wider shadow-sm">
                                Cancel
                            </button>
