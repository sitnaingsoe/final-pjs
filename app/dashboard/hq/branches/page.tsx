...
export default async function ManageBranchesPage() {
    ...
    if (!companyId) {
        return (
            <div className="p-8 text-center glass rounded-[2rem] shadow-2xl border border-red-100 mt-6 animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="space-y-6 lg:space-y-8 animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 p-6 md:p-8 glass rounded-[2rem] border border-border/50 shadow-2xl">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h1 className="text-2xl md:text-3xl font-black uppercase tracking-tight text-foreground">Store Architecture</h1>
            ...
            <div className="glass rounded-[2rem] shadow-2xl">
                <div className="flex items-center gap-3 mb-6 md:mb-8">
                    ...
                    <h2 className="text-base font-black uppercase tracking-wider text-foreground">Active Branches <span className="font-bold text-muted-foreground tracking-normal ml-1 text-sm">({branches.length})</span></h2>
                </div>
                                ...
                                <tr>
                                    <td colSpan={5} className="py-12">
                                        <div className="flex flex-col items-center justify-center text-center">
                                            ...
                                            <h3 className="text-sm font-black text-foreground uppercase tracking-widest mb-1">No Branches Found</h3>
                                            <p className="text-xs text-muted-foreground font-bold">မည်သည့်ဆိုင်ခွဲမျှ မရှိသေးပါ။</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                        ...
                                        <tr key={branch.id} className="hover:bg-black/5 transition-colors group">
                                            <td className="py-4 pl-4">
                                                <div className="flex items-start gap-4">
                                                    <div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center text-muted-foreground font-black text-sm shrink-0 group-hover:bg-black group-hover:text-white transition-colors shadow-sm">
                                                        {branch.name.substring(0, 1).toUpperCase()}
                                                    </div>
                                                    <div>
                                                        <div className="font-black text-foreground">{branch.name}</div>
                                                        <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mt-1 flex items-center gap-1.5">
                                            ...
                                            <td className="py-4">
                                                <div className="flex items-start gap-3">
                                                    <div className="w-8 h-8 rounded-full bg-card border border-border flex items-center justify-center text-muted-foreground font-black text-xs shrink-0 shadow-sm">
                                                        {manager?.name ? manager.name.substring(0, 1).toUpperCase() : '?'}
                                                    </div>
                                                    <div>
                                                        <div className="font-bold text-foreground">{manager?.name || 'မရှိပါ'}</div>
                                                        <div className="text-[10px] text-muted-foreground font-bold tracking-widest mt-0.5">{manager?.email || '-'}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-4 text-center">
                                                <div className="inline-flex items-center justify-center min-w-[3rem] px-2.5 py-1 rounded-md bg-card border border-border/50 text-muted-foreground font-black text-xs shadow-sm">
                                                    {branch._count.users}
                                                </div>
                                            </td>
                                            <td className="py-4 text-center">
                                                <div className="inline-flex items-center justify-center min-w-[3rem] px-2.5 py-1 rounded-md bg-card border border-border/50 text-foreground font-black text-xs shadow-sm">
